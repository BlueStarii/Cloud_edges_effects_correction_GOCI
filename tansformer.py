...
class tnet(Module):
    ...
    def __init__(self, ntrans=1):
        super(tnet,self).__init__()
        self.fc1 = nn.Linear(11, 512)
        self.act = nn.Tanh()
        self.act1 = nn.ReLU()
        self.act2 = nn.Sigmoid()
        # self.fct = nn.Linear(512, 8)
        self.trans = nn.ModuleList()  # Transformer(nhead=16, num_encoder_layers=12)

        for i in range(ntrans):
            self.trans.append(Transformer(nhead=4, num_encoder_layers=3))
    ...
    #     self.fct = nn.Linear(11, 8)
    #     self.act1 = nn.ReLU()
    #     self.trans = Transformer(nhead=16, num_encoder_layers=12)
    ...
    #     x = self.fc1(x)
    #     x = self.act1(x)
    #     t = self.fct(x.permute(1, 2, 0)).permute(2, 0, 1)
    #     x = self.trans(x, t)
    #     x = self.fc2(x)
...
if __name__ == "__main__":
    ...
    # src = torch.rand((12, 12, 512))
    # tgt = torch.rand((8, 12, 512))
    # out = transformer_model(src, tgt)
    # print(out.shape)
    x = torch.randn(8,1,11)
    model = tnet()
    a = model(x)
    print(a.shape)